...
/**
 * Pre-match hardware diagnostics that automatically scans and verifies all registered
 * hardware devices. Designed to be called during {@code robotInit()} before {@code waitForStart()}.
 * <p>
 * Reports results through the {@link AresFaultManager} alert system and telemetry, catching
...
public final class AresDiagnostics {
    ...
    private static final AresAlert diagnosticAlert = new AresAlert(
        "Pre-match diagnostics FAILED", AresAlert.AlertType.ERROR);
     ...
     * For each device:
     * <ul>
     *   <li><b>DcMotorEx:</b> Reads current position to confirm encoder communication.</li>
     *   <li><b>Servo:</b> Reads current position to confirm PWM communication.</li>
     *   <li><b>IMU:</b> Attempts to read angular orientation.</li>
     * </ul>
     *
    ...
    public static boolean runPreMatchCheck(HardwareMap hardwareMap) {
        ...
        // Check IMUs
        for (com.qualcomm.robotcore.hardware.IMU imu : hardwareMap.getAll(com.qualcomm.robotcore.hardware.IMU.class)) {
        ...
        if (allPassed) {
            AresAutoLogger.recordOutput("Diagnostics/Status",
                String.format("PASS %d/%d devices OK", total, total));
        } else {
            AresAutoLogger.recordOutput("Diagnostics/Status",
                String.format("FAIL %d/%d devices FAILED", failed.size(), total));
        }
