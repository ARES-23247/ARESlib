...
/**
 * A buffer that stores a history of objects that implement Interpolatable. 
 * Allows lookup of values at any given point in time (interpolated if exact timestamp isn't found).
 * 
 * Commonly used for Vision Latency Compensation (keeping a history of Pose2d).
 */
public class TimeInterpolatableBuffer<T extends Interpolatable<T>> {
     ...
     * @return the buffer.
     */
    public static <T extends Interpolatable<T>> TimeInterpolatableBuffer<T> createBuffer(int historySize) {
