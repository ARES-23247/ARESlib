...
import org.areslib.math.geometry.Pose3d;
import org.areslib.math.geometry.Rotation3d;
import org.areslib.math.geometry.Transform3d;
import org.areslib.math.geometry.Translation3d;
...
public class FrustumVisualizer {
  ...
  public static Pose3d[] generateFrustum(
    ...
    Pose3d p1 = cameraPose.transformBy(new Transform3d(topLeft, new Rotation3d()));
    Pose3d p2 = cameraPose.transformBy(new Transform3d(topRight, new Rotation3d()));
    Pose3d p3 = cameraPose.transformBy(new Transform3d(bottomLeft, new Rotation3d()));
    Pose3d p4 = cameraPose.transformBy(new Transform3d(bottomRight, new Rotation3d()));
  ...
  /**
   * Helper method to flatten a Pose3d array into the AdvantageScope primitive double array format
   * representing sequenced Quaternions: [x,y,z,w,i,j,k]
   */
  public static double[] toFlatArray(Pose3d[] poses) {
    double[] flat = new double[poses.length * 7];
    for (int idx = 0; idx < poses.length; idx++) {
      int base = idx * 7;
      flat[base] = poses[idx].getX();
      flat[base + 1] = poses[idx].getY();
      flat[base + 2] = poses[idx].getZ();
      flat[base + 3] = poses[idx].getRotation().getW();
      flat[base + 4] = poses[idx].getRotation().getX();
      flat[base + 5] = poses[idx].getRotation().getY();
      flat[base + 6] = poses[idx].getRotation().getZ();
    }
    return flat;
  }
}
