 ...
 * <p>
 * Samples robot velocities and specified binary inputs at deterministic rates
 * and exports them to a JSON file on the local filesystem.
 */
public class GhostRecorder {
     ...
     * @param booleanSuppliers A varargs array of booleans to track (e.g., intake button, shoot button).
     *                         They will be assigned IDs 0, 1, 2... based on varargs order.
     */
    @SafeVarargs
    public GhostRecorder(Supplier<ChassisSpeeds> speedsSupplier, Supplier<Boolean>... booleanSuppliers) {
    ...
    public void update() {
        ...
        m_data.pushFrame(speeds.vxMetersPerSecond, speeds.vyMetersPerSecond, speeds.omegaRadiansPerSecond, mask);
    }
