...
public class SRSHub extends I2cDeviceSynchDevice<I2cDeviceSynchSimple> {
  ...
  public abstract static class I2CDevice {
  ...
  public static class APDS9151 extends I2CDevice {
    ...
    protected void parseUpdate(BitSet data, int start) {
      ...
      byte[] proximityChunk = data.get(index, index + 11).toByteArray();

      byte[] paddedProximityChunk = new byte[2];

      System.arraycopy(proximityChunk, 0, paddedProximityChunk, 0, proximityChunk.length);

      proximity = ByteBuffer.wrap(paddedProximityChunk).order(BYTE_ORDER).getShort();

      index += 11;

      byte[] infraredChunk = data.get(index, index + 16).toByteArray();

      byte[] paddedInfraredChunk = new byte[4];

      System.arraycopy(infraredChunk, 0, paddedInfraredChunk, 0, infraredChunk.length);

      infrared = ByteBuffer.wrap(paddedInfraredChunk).order(BYTE_ORDER).getInt();

      index += 16;

      byte[] redChunk = data.get(index, index + 16).toByteArray();

      byte[] paddedRedChunk = new byte[4];

      System.arraycopy(redChunk, 0, paddedRedChunk, 0, redChunk.length);

      red = ByteBuffer.wrap(paddedRedChunk).order(BYTE_ORDER).getInt();

      index += 16;

      byte[] greenChunk = data.get(index, index + 16).toByteArray();

      byte[] paddedGreenChunk = new byte[4];

      System.arraycopy(greenChunk, 0, paddedGreenChunk, 0, greenChunk.length);

      green = ByteBuffer.wrap(paddedGreenChunk).order(BYTE_ORDER).getInt();

      index += 16;

      byte[] blueChunk = data.get(index, index + 16).toByteArray();

      byte[] paddedBlueChunk = new byte[4];

      System.arraycopy(blueChunk, 0, paddedBlueChunk, 0, blueChunk.length);

      blue = ByteBuffer.wrap(paddedBlueChunk).order(BYTE_ORDER).getInt();
    }
  }
  ...
  public static class VL53L5CX extends I2CDevice {
    ...
    public VL53L5CX(Resolution resolution) {
      config.set(0, resolution.value == 1);
    ...
    protected void parseUpdate(BitSet data, int start) {
      ...
      for (int k = 0; k < (getUpdateLength() - 1) / 11; k++) {
        byte[] chunk = data.get(index, index + 11).toByteArray();
        ...
        distances[k] = ByteBuffer.wrap(paddedChunk).order(BYTE_ORDER).getShort();
      }
    }
  }
  ...
  public static class VL53L0X extends I2CDevice {
    ...
    protected void parseUpdate(BitSet data, int start) {
      ...
      byte[] chunk = data.get(index, index + 16).toByteArray();

      byte[] paddedChunk = new byte[4];

      System.arraycopy(chunk, 0, paddedChunk, 0, chunk.length);

      distance = ByteBuffer.wrap(paddedChunk).order(BYTE_ORDER).getShort() & 0xFFFF;
    }
  }
  ...
  public static class GoBildaPinpoint extends I2CDevice {
    ...
    public static class ResetIMUCommand extends Command {
      ...
      public ResetIMUCommand(int bus) {
        ...
        data.set(1, (bus >> 1 & 1) == 1);

        int index = 2;

        for (int i = 0; i < BITS_PER_I2C_DEVICE; i++) {
          data.set(index++, (ID >> i & 1) == 1);
        }
      ...
      public SetPositionCommand(int bus, float xPosition, float yPosition, float hOrientation) {
        ...
        for (int i = 0; i < payload.length; i++) {
          for (int bit = 7; bit >= 0; bit--) {
            data.set(index++, ((payload[i] >> bit) & 1) == 1);
          }
        }
      }
    ...
    /**
     * @param xPodOffset the offset of your forward tracking pod from the tracking center in
     *     millimeters
     * @param yPodOffset the offset of your strafe tracking pod from the tracking center in
     *     millimeters
    ...
    protected void parseUpdate(BitSet data, int start) {
      ...
      byte[] deviceStatusChunk = data.get(index, index + 8).toByteArray();

      byte[] paddedDeviceStatusChunk = new byte[2];

      System.arraycopy(deviceStatusChunk, 0, paddedDeviceStatusChunk, 0, deviceStatusChunk.length);

      deviceStatus = ByteBuffer.wrap(paddedDeviceStatusChunk).order(BYTE_ORDER).getShort();

      index += 8;

      byte[] xPositionChunk = data.get(index, index + 32).toByteArray();

      byte[] paddedXPositionChunk = new byte[4];

      System.arraycopy(xPositionChunk, 0, paddedXPositionChunk, 0, xPositionChunk.length);

      xPosition = ByteBuffer.wrap(paddedXPositionChunk).order(BYTE_ORDER).getFloat();

      index += 32;

      byte[] yPositionChunk = data.get(index, index + 32).toByteArray();

      byte[] paddedYPositionChunk = new byte[4];

      System.arraycopy(yPositionChunk, 0, paddedYPositionChunk, 0, yPositionChunk.length);

      yPosition = ByteBuffer.wrap(paddedYPositionChunk).order(BYTE_ORDER).getFloat();

      index += 32;

      byte[] hOrientationChunk = data.get(index, index + 32).toByteArray();

      byte[] paddedHOrientationChunk = new byte[4];

      System.arraycopy(hOrientationChunk, 0, paddedHOrientationChunk, 0, hOrientationChunk.length);

      hOrientation = ByteBuffer.wrap(paddedHOrientationChunk).order(BYTE_ORDER).getFloat();

      index += 32;

      byte[] xVelocityChunk = data.get(index, index + 32).toByteArray();

      byte[] paddedXVelocityChunk = new byte[4];

      System.arraycopy(xVelocityChunk, 0, paddedXVelocityChunk, 0, xVelocityChunk.length);

      xVelocity = ByteBuffer.wrap(paddedXVelocityChunk).order(BYTE_ORDER).getFloat();

      index += 32;

      byte[] yVelocityChunk = data.get(index, index + 32).toByteArray();

      byte[] paddedYVelocityChunk = new byte[4];

      System.arraycopy(yVelocityChunk, 0, paddedYVelocityChunk, 0, yVelocityChunk.length);

      yVelocity = ByteBuffer.wrap(paddedYVelocityChunk).order(BYTE_ORDER).getFloat();

      index += 32;

      byte[] hVelocityChunk = data.get(index, index + 32).toByteArray();

      byte[] paddedHVelocityChunk = new byte[4];

      System.arraycopy(hVelocityChunk, 0, paddedHVelocityChunk, 0, hVelocityChunk.length);

      hVelocity = ByteBuffer.wrap(paddedHVelocityChunk).order(BYTE_ORDER).getFloat();
    }
  }
  ...
  public static class Config {
    ...
    protected final AnalogDigitalDevice[] analogDigitalDevices = new AnalogDigitalDevice[12];

    protected final Encoder[] encoders = new Encoder[6];

    @SuppressWarnings("unchecked")
    protected final ArrayList<I2CDevice>[] i2cBuses =
        new ArrayList[] {
          new ArrayList<I2CDevice>(), new ArrayList<I2CDevice>(), new ArrayList<I2CDevice>()
        };

    public Config() {
      Arrays.fill(analogDigitalDevices, AnalogDigitalDevice.NONE);

      Arrays.fill(encoders, Encoder.NONE);
    }
    ...
    public void setAnalogDigitalDevice(int pin, AnalogDigitalDevice device) {
      if (pin < 1 || pin > 12) {
        throwException(
            IndexOutOfBoundsException.class, "AnalogDigitalDevice pin " + "must be from 1 to 12");
      }

      if (locked) {
        throwException(
            IllegalStateException.class,
            "Config has already been " + "passed to the SRSHub; changes cannot be made");
      }
    ...
    public void setEncoder(int port, Encoder device) {
      if (port < 1 || port > 6) {
        throwException(IndexOutOfBoundsException.class, "Encoder port must " + "be from 1 to 6");
      }
     ...
     * @throws IllegalStateException if init has already been called or if a device of the same I2C
     *     address has been configured on the bus
     */
    public void addI2CDevice(int bus, I2CDevice device) {
      if (bus < 1 || bus > 3) {
        throwException(IndexOutOfBoundsException.class, "I2C bus must be from 1 to" + " 3");
      }
        ...
        if (i2cDevice.getAddress() == device.getAddress()) {
          throwException(
              ...
              "I2C Bus #"
                  + bus
                  + " "
                  + "already has a bus of type "
                  + i2cDevice.getClass().getName()
                  + " which has an I2C address conflicting "
                  + "with the "
                  + device.getClass().getName());
        }
      }
  ...
  public SRSHub(I2cDeviceSynchSimple deviceClient, boolean deviceClientIsOwned) {
  ...
  private void verifyInitialization() {
    if (!driverInitialized) {
      ...
      if (deviceId != DEVICE_ID) {
        RobotLog.addGlobalWarningMessage("SRSHub initialization failed");
  ...
  public void init(Config config) {
    ...
    int initLength =
        config.analogDigitalDevices.length * BITS_PER_ANALOG_DIGITAL_DEVICE
            + config.encoders.length * BITS_PER_ENCODER
            + config.i2cBuses.length * MIN_BITS_PER_I2C_BUS;
    ...
    for (int i = 0; i < config.i2cBuses.length; i++) {
      for (int j = 0; j < config.i2cBuses[i].size(); j++) {
        busLengths[i] += BITS_PER_I2C_DEVICE + config.i2cBuses[i].get(j).getInitLength();
      }
    ...
    for (int i = 0; i < config.analogDigitalDevices.length; i++) {
      ...
      for (int j = 0; j < BITS_PER_ANALOG_DIGITAL_DEVICE; j++) {
        init.set(index++, (config.analogDigitalDevices[i].value >> j & 1) == 1);
      }
    }
    ...
    for (int i = 0; i < config.encoders.length; i++) {
      ...
      for (int j = 0; j < BITS_PER_ENCODER; j++) {
        init.set(index++, (config.encoders[i].value >> j & 1) == 1);
      }
    }

    for (int i = 0; i < config.i2cBuses.length; i++) {
      for (int j = 0; j < MIN_BITS_PER_I2C_BUS; j++) {
        init.set(index++, (busLengths[i] >> j & 1) == 1);
      }
      ...
      for (int j = 0; j < config.i2cBuses[i].size(); j++) {
        ...
        for (int k = 0; k < BITS_PER_I2C_DEVICE; k++) {
          init.set(index++, (device.getValue() >> k & 1) == 1);
        }

        for (int k = 0; k < device.getInitLength(); k++) {
          init.set(index++, device.getConfig().get(k));
        }
      }
    }
    ...
    if (updateLength > 100) {
      throwException(IllegalStateException.class, "Maximum bulk-read length of 100 bytes exceeded");
    }

    byte[] data = new byte[(initLength + 7) / 8];

    System.arraycopy(init.toByteArray(), 0, data, 0, init.toByteArray().length);
    ...
    deviceClient.write(Register.INIT.address, data);
   ...
   * @throws RuntimeException if the SRSHub is unable to update according to the provided
   *     configuration
   */
  public void update() {
    if (config == null) {
      throwException(
          IllegalStateException.class, "The SRSHub must be initialized " + "before updating");
    }

    byte[] rawData = deviceClient.read(Register.READ.address, updateLength);
    ...
    int receivedCRC =
        ((rawData[rawData.length - 2] & 0xFF) << 8) | (rawData[rawData.length - 1] & 0xFF);

    int computedCRC = computeCRC16(rawData);
    ...
    for (int i = 0; i < config.analogDigitalDevices.length; i++) {
      switch (config.analogDigitalDevices[i]) {
        case ANALOG:
          byte[] chunk = data.get(index, index + 12).toByteArray();

          byte[] paddedChunk = new byte[2];

          System.arraycopy(chunk, 0, paddedChunk, 0, chunk.length);

          analogDigitalValues[i] =
              ByteBuffer.wrap(paddedChunk).order(BYTE_ORDER).getShort() / (double) 4095;
    ...
    for (int i = 0; i < config.encoders.length; i++) {
      switch (config.encoders[i]) {
        case QUADRATURE:
          ...
          byte[] quadratureChunk = data.get(index, index + 16).toByteArray();

          byte[] paddedQuadratureChunk = new byte[2];

          System.arraycopy(quadratureChunk, 0, paddedQuadratureChunk, 0, quadratureChunk.length);

          encoderValues[i].position =
              lastPosition + ByteBuffer.wrap(paddedQuadratureChunk).order(BYTE_ORDER).getShort();

          index += 16;

          byte[] quadratureVelocityChunk = data.get(index, index + 32).toByteArray();
          ...
          encoderValues[i].velocity =
              ByteBuffer.wrap(paddedQuadratureVelocityChunk).order(BYTE_ORDER).getInt();
        ...
        case PWM:
          ...
          byte[] pwmChunk = data.get(index, index + 16).toByteArray();

          byte[] paddedPWMChunk = new byte[2];

          System.arraycopy(pwmChunk, 0, paddedPWMChunk, 0, pwmChunk.length);

          encoderValues[i].position = ByteBuffer.wrap(paddedPWMChunk).order(BYTE_ORDER).getShort();

          index += 16;

          byte[] pwmVelocityChunk = data.get(index, index + 32).toByteArray();

          byte[] paddedPWMVelocityChunk = new byte[4];

          System.arraycopy(pwmVelocityChunk, 0, paddedPWMVelocityChunk, 0, pwmVelocityChunk.length);

          encoderValues[i].velocity =
              ByteBuffer.wrap(paddedPWMVelocityChunk).order(BYTE_ORDER).getInt();
   ...
   * @return the current value read from the AnalogDigitalDevice; from 0 to 1 for analog devices and
   *     0 or 1 for digital devices
   * @throws IndexOutOfBoundsException if the pin is not between 1 and 12, inclusive
   * @throws IllegalStateException if the SRSHub has not yet been initialized
  ...
  public double readAnalogDigitalDevice(int pin) {
    ...
    if (config == null) {
      throwException(
          IllegalStateException.class, "The SRSHub must be initialized " + "before reading");
    }

    if (config.analogDigitalDevices[pin - 1] == AnalogDigitalDevice.NONE) {
      throwException(
          IllegalStateException.class, "AnalogDigitalDevice pin #" + pin + " was not configured");
    }
   ...
   * @return the current position and velocity of the encoder; for quadrature encoders this is in
   *     ticks/ticks per second; for PWM encoders this is in pulse width (microseconds/microseconds
   *     per second)
   * @throws IndexOutOfBoundsException if the port is not between 1 and 6, inclusive
   * @throws IllegalStateException if the SRSHub has not yet been initialized
  ...
  public PosVel readEncoder(int port) {
    ...
    if (config.encoders[port - 1] == Encoder.NONE) {
      throwException(IllegalStateException.class, "Encoder port #" + port + " was not configured");
    }
  ...
  public <T extends I2CDevice> T getI2CDevice(int bus, Class<T> deviceClass) {
    ...
    if (config == null) {
      throwException(IllegalStateException.class, "The SRSHub must be initialized before reading");
    }
    ...
    throwException(
        IllegalStateException.class,
        "I2C device " + deviceClass.getName() + " was not configured on bus #" + bus);
  ...
  public void runCommand(Command command) {
    ...
    System.arraycopy(
        command.getData().toByteArray(), 0, data, 0, command.getData().toByteArray().length);

    deviceClient.write(Register.COMMAND.address, data);
  }
}
