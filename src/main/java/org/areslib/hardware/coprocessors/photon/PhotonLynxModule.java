...
/**
 * A customized LynxModule implementation for intercepting and routing LynxCommands through the
 * PhotonCore parallel transmission pipeline to optimize FTC hardware loop times.
 */
@SuppressWarnings({"rawtypes"})
public class PhotonLynxModule extends LynxModule {
  private ArrayList<LynxMessage> skippedAcquire = new ArrayList<>();

  public PhotonLynxModule(
      LynxUsbDevice lynxUsbDevice, int moduleAddress, boolean isParent, boolean isUserModule) {
    super(lynxUsbDevice, moduleAddress, isParent, isUserModule);
  }

  /**
   * Gets the map of commands currently awaiting a response from the Hub.
   *
   * @return Concurrent map of sequence IDs to responding commands
   */
  ...
  public void sendCommand(LynxMessage command)
    ...
    if (command instanceof LynxCommand) {
      if (PhotonCore.getCacheResponse((LynxCommand) command) != null) {
        ((LynxCommand) command)
            .onResponseReceived(PhotonCore.getCacheResponse((LynxCommand) command));
        return;
      }
