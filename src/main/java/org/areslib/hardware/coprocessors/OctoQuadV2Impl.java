...
public class OctoQuadV2Impl implements AresOctoQuadDriver {
    ...
    public void clearReadCache() {
        // OctoQuadFWv3 reads bulk data on-demand over I2C and does not expose a public clearCache method directly.
    }
    ...
    public void setChannelBankConfig(int channel, OctoMode mode) {
        if (octoQuadV2 != null) {
            ...
            // Just assume we want ALL_PULSE_WIDTH if any channel needs absolute, this is a simplified wrapper
            ...
            if (mode == OctoMode.PULSE_WIDTH || mode == OctoMode.ABSOLUTE) {
                if (channel < 4) {
                    if (cfg == OctoQuadFWv3.ChannelBankConfig.ALL_QUADRATURE) {
                        octoQuadV2.setChannelBankConfig(OctoQuadFWv3.ChannelBankConfig.BANK1_QUADRATURE_BANK2_PULSE_WIDTH);
                    }
                } else {
                    if (cfg == OctoQuadFWv3.ChannelBankConfig.ALL_QUADRATURE) {
                        octoQuadV2.setChannelBankConfig(OctoQuadFWv3.ChannelBankConfig.BANK1_QUADRATURE_BANK2_PULSE_WIDTH);
