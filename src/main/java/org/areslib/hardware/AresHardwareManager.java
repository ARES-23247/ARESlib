 ...
 *
 * <p>This class abstracts the initialization and persistent state of advanced FTC coprocessors such
 * as the OctoQuad (V1/V2) and custom SRS Hubs. It also manages system-level power inputs like
 * battery voltage and floodgate (current) sensors, computing a safe dynamic power scaling factor to
 * prevent robot brownouts.
 */
public class AresHardwareManager {
  ...
  /**
   * Initializes the static hardware dependencies required for the ARESLib environment. This method
   * automatically attempts to detect high-performance coprocessors and sets up the system power
   * monitoring.
   *
  ...
  public static void initHardware(HardwareMap hardwareMap) {
    ...
    try {
      ...
      org.areslib.hardware.coprocessors.SRSHub.Config srsConfig =
          new org.areslib.hardware.coprocessors.SRSHub.Config();
      for (int i = 1; i <= 6; i++) {
        srsConfig.setEncoder(i, org.areslib.hardware.coprocessors.SRSHub.Encoder.QUADRATURE);
      }
      for (int i = 1; i <= 12; i++) {
        srsConfig.setAnalogDigitalDevice(
            i, org.areslib.hardware.coprocessors.SRSHub.AnalogDigitalDevice.ANALOG);
      }
    ...
    try {
      ...
      floodgateSensor =
          new org.areslib.hardware.wrappers.AnalogInputWrapper(
      ...
      if (activeSrsHub != null) {
        floodgateSensor =
            new org.areslib.hardware.wrappers.AresSrsSensor(
                1, org.areslib.hardware.wrappers.SrsMode.ANALOG);
      } else {
  ...
  /**
   * Calculates and updates the dynamic power scaling metrics based on current power draw and system
   * voltage. Updates the underlying logging inputs to telemetry.
   */
   ...
   * @deprecated This method is non-functional and always returns null. Will be removed in a future
   *     version.
   */
  @Deprecated
  public static Object getSrsI2cBus(int bus) {
    // Obsolete functionality, we don't bind embedded i2c inside the srs hub wrapper for simplicity
    // currently.
    return null;
  }
