...
public class LimelightVisionWrapper implements VisionIO {
  ...
  public void updateInputs(VisionInputs inputs) {
    ...
    inputs.fiducialCount =
        bestResult.getClassifierResults() != null
            ? bestResult.getClassifierResults().size()
    ...
    // Process Botpose (Returns WPI field-centric pose natively if Limelight is configured for
    // Megatag)
    // Note: No coordinate inversion is needed here (unlike Odometry) because Limelight's internal
    // engine uses the FRC +X Forward, +Y Left standard automatically.
    ...
    // Loop again to package ALL valid individual camera poses into rawCameraPoses for
    // visualization.
    int validCount = 0;
    for (Limelight3A limelight : limelights) {
    ...
    for (Limelight3A limelight : limelights) {
      if (!limelight.isConnected()
          || limelight.getLatestResult() == null
          || !limelight.getLatestResult().isValid()) continue;
