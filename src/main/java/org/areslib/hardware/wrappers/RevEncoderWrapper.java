...
/**
 * Wrapper for the REV Through Bore Encoder used in Absolute Mode (PWM). Since standard FTC Control
 * Hubs cannot natively read high-frequency PWM inputs, this wrapper expects to receive the pulse
 * width (in microseconds) as determined by a device like an OctoQuad, SRS Hub, or custom hardware
 * interface.
 */
public class RevEncoderWrapper implements AresAbsoluteEncoder {
   ...
   *
   * @param pulseWidthUsSupplier A supplier returning the PWM pulse width in microseconds (e.g.,
   *     from an OctoQuad).
   ...
   */
  public RevEncoderWrapper(
      DoubleSupplier pulseWidthUsSupplier, EncoderVersion version, boolean inverted) {
