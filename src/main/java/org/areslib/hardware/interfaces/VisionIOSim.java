 ...
 *   <li>Configurable latency injection</li>
 *   <li>Range-limited detection (tags beyond maxRange are invisible)</li>
 * </ul>
 * <p>
 * The robot's true pose is read from the first non-infinite-mass body in the
 * physics world (i.e., the robot chassis), and synthetic {@code botPose3d}
 * quaternion arrays are generated in AdvantageScope-compatible format.
 *
 * @see VisionIO
...
public class VisionIOSim implements VisionIO {
    ...
    /**
     * Represents a known AprilTag location on the field.
     * Coordinates are in meters with origin at field center (vision coordinate system).
     */
    ...
    public void updateInputs(VisionInputs inputs) {
        ...
        inputs.ty = Math.toDegrees(Math.atan2(bestTag.z - 0.2, bestDistance)); // Assume camera at 0.2m height
        ...
        double mt2Heading = robotHeading + rng.nextGaussian() * config.headingNoiseStdDev * mt2NoiseFactor;
        double mt2Half = mt2Heading / 2.0;
        inputs.botPoseMegaTag2 = new double[]{mt2X, mt2Y, 0.0, Math.cos(mt2Half), 0.0, 0.0, Math.sin(mt2Half)};
    ...
    /**
     * Finds the robot chassis body in the physics world.
     * The robot is identified as the first body with NORMAL mass type (non-infinite, non-artifact).
     * Artifacts are distinguished by having circular fixtures and low mass.
     *
