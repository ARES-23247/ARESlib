...
public class Commands {
   ...
   */
  public static Command sequence(Command... cmds) {
    return new SequentialCommandGroup(cmds);
  }
   ...
   */
  public static Command either(
      Command onTrue, Command onFalse, java.util.function.BooleanSupplier sel) {
   ...
   */
  public static Command runOnce(Runnable r) {
  ...
  /**
   * Creates a command that executes the provided runnable periodically and never finishes until
   * interrupted.
   *
  ...
  /**
   * Creates a command that builds the inner command dynamically at runtime rather than at
   * definition time.
   *
  ...
  public static Command defer(java.util.function.Supplier<Command> s) {
    return new Command() {
      ...
      public void initialize() {
        inner = s.get();
        if (inner != null) inner.initialize();
      }
