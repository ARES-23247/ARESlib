 ...
 * <p>
 * Validates the full subsystem-level behavior: kinematics decomposition,
 * module state optimization, wheel speed desaturation, slew rate limiting,
 * field-centric transform, and zero-speed anti-snap behavior.
 * <p>
 * Uses {@link SwerveModuleIOSim} for all 4 modules — no hardware or physics world needed.
 */
public class SwerveDriveSubsystemTest {
    ...
    void testFieldCentricDriveTransformsHeading() {
        ...
        assertTrue(Math.abs(vx) < 0.1 || Math.abs(vy) > 0.5,
    ...
    void testSlewRateLimitingSmooths() {
        ...
        assertTrue(commandedVx < 1.0,
            "Slew rate limiter should prevent instant jump to 10 m/s. Got: " + commandedVx);
        assertTrue(commandedVx > 0.0,
            "Slew rate limiter should allow SOME movement. Got: " + commandedVx);
    }
