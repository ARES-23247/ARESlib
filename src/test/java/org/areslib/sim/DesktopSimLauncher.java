...
public class DesktopSimLauncher {
  ...
  public static void main(String[] args) {
    ...
    // Since isSimulation is true, hardwareMap and gamepads can be safely passed as null or mock
    // values
    ...
    org.areslib.hardware.wrappers.ArrayLidarIOSim lidarSim =
        new org.areslib.hardware.wrappers.ArrayLidarIOSim(() -> odometryInputs, world);
    org.areslib.hardware.interfaces.ArrayLidarIO.ArrayLidarInputs lidarInputs =
    ...
    org.dyn4j.dynamics.BodyFixture fixture =
        new org.dyn4j.dynamics.BodyFixture(
            Geometry.createRectangle(
                FieldConstants.ROBOT_SIZE_METERS, FieldConstants.ROBOT_SIZE_METERS));
      ...
      while (true) {
        ...
        if (!isAutoEnabled) {
          ...
          org.areslib.math.geometry.Rotation2d allianceHeading =
              currentHeading.rotateBy(
                  new org.areslib.math.geometry.Rotation2d(
                      currentAlliance.getHeadingOffsetRadians()));

          if (driveSubsystem instanceof org.areslib.subsystems.drive.SwerveDriveSubsystem) {
            ((org.areslib.subsystems.drive.SwerveDriveSubsystem) driveSubsystem)
                .driveFieldCentric(driveY, driveX, driveTurn, allianceHeading);
          } else if (driveSubsystem instanceof org.areslib.subsystems.drive.MecanumDriveSubsystem) {
            ((org.areslib.subsystems.drive.MecanumDriveSubsystem) driveSubsystem)
                .driveFieldCentric(driveY, driveX, driveTurn, allianceHeading);
          } else if (driveSubsystem
              instanceof org.areslib.subsystems.drive.DifferentialDriveSubsystem) {
            ((org.areslib.subsystems.drive.DifferentialDriveSubsystem) driveSubsystem)
                .drive(driveY, driveTurn);
          }
        }
        ...
        double forceX =
            (vXFieldTarget - currentVx) * driveTractionGrip * robotBody.getMass().getMass();
        double forceY =
            (vYFieldTarget - currentVy) * driveTractionGrip * robotBody.getMass().getMass();
        double torque =
            (omega - currentOmega) * turnTractionGrip * robotBody.getMass().getInertia();
